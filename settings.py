...
def set_sprite(sprite_list, number, name, color, width, height, folder):
    '''
    ...
    folde : tupe string
        Папка, из которой берутся спрайты.

    Returns None.
    -------
    '''
    img_dir = path.join(path.dirname(__file__), folder)
...
def load_status():
    ...
    with open("text.txt") as file:
        for line in file:
            status.append(float(line.split()[2]))
    return(status)
...
set_dic = {'WIDTH' : WIDTH, 'HEIGHT' : HEIGHT, 'FPS' : FPS, 'WHITE' : WHITE, 
           ...
           'gachi_mode_activate' : load_status()[8]}

# Задаем папку, из которой будем брать все звуки.
if load_status()[8] == 0:
    snd_dir = path.join(path.dirname(__file__), 'sound')
else:
    snd_dir = path.join(path.dirname(__file__), 'sound_gachi')
